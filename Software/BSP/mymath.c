...
char buffer[2] = {0}; //用于存放转换好的十六进制字符串。由于只要支持2位整数，所以长度3即可
...
char inttohex(int aa)
{
	...
	j = 0;
	aa *= 16;
	while (aa >= 16) //把转换好的十六进制数依次输入数组
	{
		i = ((aa / 16) % 16); //先是求余
		buffer[j] = x[i];	  //把得到的余数转为十六进制数（例如“11”转“b”）
		j++;				  //数组下标移位
		aa /= 256;			  //求商再赋值给它自己（方便下个循环再除）
		if (aa < 16)
			buffer[j] = x[aa];
	}

	return (CharToHex(buffer[1]) << 4 | CharToHex(buffer[0]));
}
